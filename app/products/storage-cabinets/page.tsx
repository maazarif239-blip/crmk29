...
export default async function StorageCabinets() {
  const supabase = await createClient();

  const { data: category } = await supabase
    .from('categories')
    .select('id')
    .eq('slug', 'storage-cabinets')
    .single();
  ...
  return (
    <div className="min-h-screen w-full min-w-0 overflow-x-clip bg-white text-gray-900 font-sans selection:bg-[#E5E0D8]">
      <ProductPageHeader title="Storage Cabinets" description="Precision-crafted storage solutions for the modern professional. Discover cabinets designed for secure organization, enduring quality, and seamless workspace integration." />
