...
export default function TeamPage() {
  ...
  const saveTeam = async () => {
    ...
    if (invalidMembers.length > 0) { alert('All team members must have a name'); return; }

    setSaving(true);
    const supabase = createClient();
    const { error } = await supabase
      .from('site_content')
      .upsert({ key: 'management_team', value: team }, { onConflict: 'key' });
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64"><p className="text-gray-500">Loading team members...</p></div>;
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={addTeamMember} className="flex items-center gap-2 bg-[#EB5324] text-white px-4 py-2 rounded-lg hover:bg-[#d4481f] transition-colors">
          ...
          team.map((member, index) => (
            <div key={index} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <button onClick={() => deleteTeamMember(index)} className="flex items-center gap-2 text-red-600 hover:text-red-700 transition-colors">
                  ...
                  <div className="flex flex-col gap-3">
                    {member.image_url ? (
                      <div className="relative w-full aspect-square rounded-lg overflow-hidden border-2 border-gray-200">
                        <img src={member.image_url} alt={member.name || 'Team member'} className="w-full h-full object-cover" />
                      </div>
                    ) : (
                    ...
                    <label className="cursor-pointer">
                      <div className="flex items-center justify-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg transition-colors">
                        ...
                        <span className="text-sm font-medium">{uploadingIndex === index ? 'Uploading...' : 'Upload Photo'}</span>
                      </div>
                      <input
                        type="file"
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => { const file = e.target.files?.[0]; if (file) handleImageUpload(index, file); }}
      ...
      {team.length > 0 && (
        <div className="flex justify-end">
          <button onClick={saveTeam} disabled={saving} className="bg-green-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
